...
def is_list_empty(lst):
    # 检查列表是否为空，或列表中的所有字符串是否为空
    return not lst or all(s == '' for s in lst)
...
def Amazon(data_file, rating_score):
    ...
    all_num, wo_rating_num,wo_review_num,wo_summary_num= 0, 0 ,0 ,0
    for inter in parse(data_file):
        ...
        if float(inter['overall']) <= rating_score:  # 小于一定分数去掉
            continue
        if 'reviewText' not in inter:
            wo_review_num += 1
            continue
        if 'summary' not in inter:
            wo_summary_num += 1
            continue
        ...
        review = inter['reviewText']
        summary = inter['summary']
        datas.append((user, item, int(time), rating,review,summary))
    print('total review', all_num, 'review w/o rating', wo_rating_num, wo_rating_num / all_num,
    'review w/o review', wo_review_num, wo_review_num / all_num,
    'review w/o summary', wo_summary_num, wo_summary_num / all_num
    )
    return datas
...
def Amazon_meta(meta_file, data_maps):  # return the metadata of products
    ...
    for info in parse(meta_file):
        ...
        if is_list_empty(info['description']):
            no_desc +=1
        

        if info['description']==[] or info['description']==['']:
        ...
        if new_info['title'] == 'EMINEM SHOW' or is_list_empty(info['description']) or info['price'] =='' or is_no_rank:
            continue
    ...
    print(no_desc/all_num)
    print(no_price/all_num)
    print(no_rank/all_num)
    meta_set = set(datas.keys())
    return datas, item_asins.difference(meta_set)


# categories and brand  des,price,rank,is all attribute
def get_attribute_Amazon(meta_infos, datamaps, attribute_core):
    ...
    for iid, info in meta_infos.items():
        ...
        if isinstance(info['rank'], list):
            ...
            attributes[info['rank']] += 1

        try:
            attributes[info['brand']] += 1
            
            attributes[info['price']] += 1
        except:
            pass
    ...
    for iid, info in meta_infos.items():
            ...
            if isinstance(info['rank'], list):
                for rank in info['rank']:
                    if rank=="":
                        ...
                        new_meta[iid].append(rank)
                        break
            else:
                
                new_meta[iid].append(info['rank'])
        except:
            new_meta[iid].append('unknown rank')

        try:
            new_meta[iid].append(info['price'])
        except:
            new_meta[iid].append('unknown price')
        for cate in info['categories']:
        ...
        for description in info['description']:
            if description=="":
                ...
                new_meta[iid].append(description)
                break
        if len(new_meta[iid]) < 5:
            print()
        assert len(new_meta[iid]) == 5
        if len(new_meta[iid]) > 5:
            print(new_meta[iid], attributes[info['brand']], info['description'],info['rank'],attributes[info['price']],info['categories'])
    # mapping
    attribute2id = {}
    id2attribute = {}
...
def get_interaction(datas):  # return a dict, key is user and value is a list of items
    user_seq = {}
    for data in datas:
        user, item, time, rating,review,summary = data
        if user in user_seq:
            user_seq[user].append((item, time, rating,review,summary))
        else:
            user_seq[user] = []
            user_seq[user].append((item, time, rating,review,summary))
    ...
    while not isKcore:
        for user, num  in user_count.items():
            if user_count[user] < user_core:  # delete user
                ...
                for item, rating,review, summary in user_items[user]:
                    if item_count[item] < item_core:
                        user_items[user].remove([item, rating,review,summary])
        user_count, item_count, rating_count, isKcore = check_Kcore(user_items, user_core, item_core)
    return user_items
...
def id_map(user_items):  # user_items dict
    ...
    for user in random_user_list:
        ...
        reviews = []
        summaries= []
        for item, rating,review,summary in items:
        ...
        final_data[uid] = [iids, ratings,reviews,summaries]
    data_maps = {
        'user2id': user2id,
        'item2id': item2id,
        'id2user': id2user,
...
def update_data(user_items, item_diff, id2item):
    ...
    for user, user_data in user_items.items():
        iids, ratings,reviews,summaries = user_data
        new_idds, new_ratings,new_reviews,new_summaries = [], [],[],[]
        for id, rating,review,summary in zip(iids, ratings,reviews,summaries):
            if id2item[id] not in item_diff:
                new_idds.append(id)
                new_ratings.append(rating)
                new_reviews.append(review)
                new_summaries.append(summary)
        new_data[user] = [new_idds, new_ratings,new_reviews,new_summaries]
...
def preprocess(data_file, meta_file, processed_dir, data_type='Amazon'):
    ...
    for itemid, title in sample_item:
        brand,rank,price,cate,description= item2attributes[itemid]
        #brand,rank,price,cate,description
        print('Title:', title)
        print('Brand:', datamaps['id2attribute'][brand])
        print('Category:', datamaps['id2attribute'][cate])
        print('description:', datamaps['id2attribute'][description])
        print('rank:', datamaps['id2attribute'][rank])
        print('price:', datamaps['id2attribute'][price])
...
if __name__ == '__main__':
    set_seed(1234)
    DATA_DIR = '/mnt/nasSave/user/pengze/data/'
    DATA_SET_NAME = 'amz'
    SUBSET_NAME = 'Books'
    SHORT_NAME = 'books'
    ...
    PROCESSED_DIR = os.path.join(DATA_DIR, DATA_SET_NAME, 'bloomz_amz_all_itemSide_100w')
